...
it('transfers token ownership',function(){
  return DappToken.deployed().then(function(instance){
    tokenInstance = instance;
    // Test 'require' statement first by transferring soemthing larger than the sender's adminBalance
    ...
    assert.equal(receipt.logs[0].args._from,accounts[0],'logs the account the tokens are tranferred from');
    ...
    assert.equal(balance.toNumber(), 250000,'adds the amount to the receicing account');
    return tokenInstance.balanceOf(accounts[0]);
  }).then(function(balance){
...
it('appproves tokens for delegated transfer', function(){
  return DappToken.deployed().then(function(instance){
    tokenInstance = instance;
...
it('handles delegated token transfers', function(){
  return DappToken.deployed().then(function(instance){
    ...
    assert(error.message.indexOf('revert')>=0,'cannot tranfer value larger than the balance');
